...
const getCard = async (req, res) => {
    ...
    const card = await db.collection(cardsCollection).findOne({ _id: new ObjectId(req.params.id) })
...
const updateCard = async (req, res) => {
    ...
    await db.collection(cardsCollection).updateOne({ _id: new ObjectId(req.params.id) }, { $set: value })
...
const deleteCard = async (req, res) => {
    ...
    const column = await db.collection(columnsCollection).findOne({ cards: { _id } })
    column.cards = column.cards.filter((crd) => crd._id.toString() !== card._id.toString())

    await db.collection(columnsCollection).updateOne({ _id: column._id }, { $set: column })
    await db.collection(cardsCollection).deleteOne({ _id: new ObjectId(req.params.id) })
